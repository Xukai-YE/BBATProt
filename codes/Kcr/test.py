...
test = test.reshape(-1,31,512)
print('test load feature done!',test.shape)
sys.stdout.flush()
# ------------------------------------------- load model and test
print('load model……')
model = tf.keras.models.load_model('BERT_model-BBAT3.h5', custom_objects={'Attention3D':Attention3D, 'TCN': TCN})
...
# np.savetxt('test_realuts_BBATProt2.txt',yy_pred,fmt='%s')
true_values = np.array(test_label)
# np.savetxt('true_labels_BBATProt2.npy', true_values)
